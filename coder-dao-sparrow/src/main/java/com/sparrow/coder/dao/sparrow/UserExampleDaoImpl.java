...
package com.sparrow.coder.dao.sparrow;

import com.sparrow.orm.query.*;
import com.sparrow.orm.template.impl.ORMStrategy;
import com.sparrow.coder.dao.UserExampleDAO;
...
public class UserExampleDaoImpl extends ORMStrategy<UserExample, Long> implements UserExampleDAO {
    ...
    private BooleanCriteria generateCriteria(UserExampleDBPagerQuery userExampleQuery) {
        BooleanCriteria booleanCriteria = BooleanCriteria.criteria(Criteria.field(UserExample::getUserName).equal(userExampleQuery.getUserName()))
            .and(Criteria.field(UserExample::getChineseName).equal(userExampleQuery.getChineseName()))
            .and(Criteria.field(UserExample::getCreateUserId).equal(SessionContext.getLoginUser().getUserId()));
        if (userExampleQuery.getStatus() != null && userExampleQuery.getStatus() >= 0) {
            booleanCriteria.and(Criteria.field(UserExample::getStatus).equal(StatusRecord.valueOf(userExampleQuery.getStatus())));
        }
