...
CREATE TABLE `t_table_config` (
 ...
 `checkable` tinyint(4)  default 0 comment '是否可勾选 -1:不可 >=0:列序号'  NOT NULL,
 `row_menu` tinyint(1)  default 0 comment '是否显示行操作 -1：不显示 >=0:列序号'  NOT NULL,
 `column_filter` tinyint(1)  default 0 comment '是否显示列过滤器 -1:不显示 >=0:列序号'  NOT NULL,
